...
## Classes
Lightweight classes are supported (see `airport.neb`).
```c#
class klass
{
...
## Time
Musical and midi time uses the `MusicTime` class. See [MidiLib](https://github.com/cepthomas/MidiLib/blob/main/README.md).

Nebulator doesn't care about measures, that's up to you.
...
## Musical Terminology
See [MusicLib](https://github.com/cepthomas/MusicLib/blob/main/README.md).
...
```
A simple include mechanism is supplied. It compiles and links the included file, just like the main file.
...
## Properties shared by host and script

```c#
bool Playing
```
Indicates that script is executing. Script read only.

```c#
MusicTime StepTime
```
Current Nebulator sequence time. Script read only.

```c#
double RealTime
```
Seconds since playing started. Script read only.

```c#
double Tempo
```
Nebulator speed in BPM. Script read/write.

```c#
int MasterVolume
```
Nebulator master volume. Script read/write.
...
### Script implemented functions called by host
These are overridden in the user script.
...
```c#
public override void Step(); Required.
```
Called every tick.
...
- chname: channel to send it on
- note: as int or string
- vol: volume from 0.0 to 1.0 (0.0 means note off)
- dur: ow long it lasts in beat.eigth-beat or MusicTime representation
...
- chname: channel to send it on
- ctl: controller name from the definitions
...
- scale_or_chord: name of one of the builtin named scales or from CreateNotes()
- key: root note and octave
